...
export async function equipItem(itemId: string, slot: EquipmentSlot): Promise<EquippedEntry[]> {
  const response = await httpClient.post<EquippedEntry[]>('/characters/me/equipment', { itemId, slot });
  return response.data;
}
...
export async function sellItem(itemId: string, quantity = 1): Promise<{ proceeds: string }> {
  const response = await httpClient.post<{ proceeds: string }>(`/characters/me/inventory/${itemId}/sell`, { quantity });
  return response.data;
}
